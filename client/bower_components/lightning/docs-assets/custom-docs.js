...
docsApp.controller.DocsController = function($scope, $location, $window, sections, $filter) {
    ...
    $scope.afterPartialLoaded = function() {
        ...
        if($scope.currentPage.type == 'directive'){
            $scope.partialTitle = $filter('colonDashCase')($scope.currentPage.shortName);
        } else {
    ...
    $scope.$watch(function docsPathWatch() {return $location.path(); }, function docsPathWatchAction(path) {
        ...
        if (partialId) {
            ...
            if (partialId == 'angular.Module') {
                ...
                breadcrumb.push({ name: match[1], url: sectionPath + '/' + match[1] });
                breadcrumb.push({ name: match[2], directive:true });
            ...
            terms && angular.forEach(terms.toLowerCase().split(' '), function(term) {
                ...
                if (ranking) {
                    if((index = title.indexOf(term)) != -1){
                        ranking = 100
                    } else if (keywords.indexOf(term) == -1) {
                        ranking = null;
                    } else {
...
angular.module('lightning').requires.push('hljs');

angular.module('lightning').directive('tkExample', function(){
    return {
        transclude:true,
        restrict:"E",
        replace:true,
        template:"<div class='bs-example' ng-transclude></div>"
    }
})

angular.module('lightning').directive('tkCodeWrapper', function(){
    return {
        transclude:true,
        restrict:"E",
        replace:true,
        template:"<figure class='highlight'><pre ng-transclude></pre></figure>"
    }
})