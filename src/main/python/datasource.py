...
class DataSourceConnection:
    ...
    def _datasource_http(self, data, method='POST'):
        ...
        if datasource is None:
            raise ValueError(
                'cannot find datasource {} even though it was created'.format(datasource_id))
...
class DataSource:
    ...
    def is_running(self):
        result = json_http(
            '{}/{}'.format(self.connection.get_jobs_url(), self.datasource_id()), method='GET')
    ...
    def start(self):
        ...
        json_http('{}/{}'.format(self.connection.get_jobs_url(),
                                 self.datasource_id()), method='POST')
    ...
    def stop(self):
        ...
        json_http('{}/{}'.format(self.connection.get_jobs_url(),
                                 self.datasource_id()), method='DELETE')
...
def main():
    ...
    parser_create = subparsers.add_parser(
        'create_web', help='create a web datasource')
    parser_create.add_argument(
        '--connector', help='the connector', default='lucid.anda')
    parser_create.add_argument(
        '--pipeline', help='the pipeline', default='conn_solr')
    parser_create.add_argument(
        '--start-url', dest='start_urls', help='start URL', action='append', required=True)
    parser_create.add_argument(
        '--depth', type=int, help='depth to crawl', default=2)
    parser_create.add_argument(
        '--restrictToTree', type=bool, help='restrict to tree', default=True)
    parser_create.add_argument(
        '--include-regexp', dest='include_regexps', help='include regexps', action='append')
    ...
    url = 'http://{}:{}/connectors/api/v1/connectors/datasources'.format(
        args.host, args.port)
    logger.info('url={}'.format(url))

    common_arg_names = ['host', 'port', 'func', 'which_parser', 'debug']
    sub_args = {key: vars(args)[key]
                for key in vars(args) if not key in common_arg_names}
    command = DataSourcesCommand(url)

    logger.debug(
        'calling {} with arguments {}'.format(args.which_parser, sub_args))
