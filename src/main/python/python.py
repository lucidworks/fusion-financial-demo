...
def standard(name=None):
    ...
    current_url = url_for(
        'standard', start=str(start), q=query, fq=fq, active=active)
    results_url = url_for(
        'standard', start=str(start), q=query, fq=fq, active='Results')
    historical_url = url_for(
        'standard', start=str(start), q=query, fq=fq, active='Historical')
    next_url = url_for(
        'standard', start=str(next_start), q=query, fq=fq, active=active)
    prev_url = url_for(
        'standard', start=str(prev_start), q=query, fq=fq, active=active)
...
def load_users(baseDN='ou=People,dc=grantingersoll,dc=com'):
    ...
    try:
        ldap_result_id = the_ldap.search(
        ...
        while 1:
            ...
            if (result_data == []):
                ...
                if result_type == ldap.RES_SEARCH_ENTRY:
                    ...
                    # print "id: " + result_data[0][0] + " val: " +
                    # result_data[0][1]["uid"][0]
                    user = {'id': result_data[0][0], 'uid': result_data[0][1][
                        'uid'][0], 'given_name': result_data[0][1]['givenName'][0]}
...
def create_filters(filters, users):
    ...
    for the_filter in filters_split:
        ...
        print data
        result = lweutils.json_http(
            COL_URL + '/roles', method='POST', data=data)
        print 'Result:'
        print result
...
if __name__ == '__main__':
    ...
    p.add_option(
        '--api_host', action='store', dest='host', default='localhost')
    # create the collection
    p.add_option('-l', '--collection', action='store', dest='collection')
    p.add_option('--api_port', action='store', dest='api_port', default='8888')
    p.add_option(
        '--ui_host', action='store', dest='ui_host', default='localhost')
    # Most people are on 389
    p.add_option('--ldap', action='store', dest='ldap')
    p.add_option('--ldapuser', action='store', dest='ldap_user',
                 default='cn=Directory Manager,cn=Root DNs,cn=config')
    ...
    p.add_option('--baseDN', action='store', dest='base_dn',
                 default='ou=People,dc=grantingersoll,dc=com')
